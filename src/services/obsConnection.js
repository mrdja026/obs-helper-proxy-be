...
class OBSConnectionManager {
    ...
    try {
      if (this.connected) {
        return {
          ...
          message: "Already connected to OBS",
        };
      }
      ...
      throw new OBSConnectionError(
        `Failed to connect to OBS: ${error.message}`
      );
    }
  }
  ...
  setupEventListeners() {
    ...
    this.obs.on("ConnectionClosed", async () => {
      ...
      if (this.connectionAttempts < this.maxReconnectAttempts) {
        ...
        logger.info(
          `Attempting to reconnect (${this.connectionAttempts}/${this.maxReconnectAttempts})...`
        );
      ...
      // Notify subscribers about connection status change
      this.notifyEventHandlers("connectionStatus", {
        connected: false,
        timestamp: new Date().toISOString(),
      });
  ...
  async getCurrentScene() {
    try {
      ...
      throw new OBSConnectionError(
        `Failed to get current scene: ${error.message}`
      );
    }
  }
  ...
  async getInputMuteStatus(inputName = config.obs.micInputName) {
    try {
      ...
      logger.error("Error getting input mute status:", error);
      throw new OBSConnectionError(
        `Failed to get input mute status: ${error.message}`
      );
    }
  }
  ...
  async setInputMute(inputName = config.obs.micInputName, inputMuted) {
    try {
      ...
      logger.error("Error setting input mute:", error);
      throw new OBSConnectionError(
        `Failed to set input mute: ${error.message}`
      );
    }
  }
  ...
  async toggleInputMute(inputName = config.obs.micInputName) {
    try {
      ...
      logger.error("Error toggling input mute:", error);
      throw new OBSConnectionError(
        `Failed to toggle input mute: ${error.message}`
      );
    }
  }
